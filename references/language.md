...
0 and the program runs. There are eight, and each has a **name**, which is what
...
`unused_item` reports a `fun`, `struct`, `enum` or `trait` that no *live*
declaration names. It is reachability rather than "was it named": a function
