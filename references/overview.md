...
backend (bytecode + stack VM) currently executes the value-type subset, with a
garbage collector planned for heap objects.
...
| Path | Contents |
|---|---|
| `src/`, `include/` | the compiler + VM, one `.c`/`.h` pair per component |
...
- **Executes (`--run`):** Int/Float/Bool/unit/ranges/functions — arithmetic,
  control flow, recursion, first-class function values, `print`. Strings,
  arrays, structs, enums, match, and closures compile but are rejected by
  codegen with "not supported by the VM yet" until the GC lands. See
  `runtime.md`.
